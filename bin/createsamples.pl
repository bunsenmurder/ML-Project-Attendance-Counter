...
# Replaced less then or equal to
for (my $k = 0; $k <= $#positives; $k++ ) {
    ...
    !chomp($img);
# Added minus 4 to substr method, as it seems later versions of perl use diffrent implementation.
    my $vec = $outputdir . substr($img, $imgdirlen, -4) . ".vec" ;
